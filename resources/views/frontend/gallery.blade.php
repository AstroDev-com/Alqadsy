    ...
    <div class="container py-5">
        ...
        <div class="dense-gallery" id="lightgallery">
            @forelse ($products as $info)
                ...
                @endphp
                <div class="item" data-aos="fade" data-src="{{ $imageUrl }}" data-sub-html="<h4>{{ $info->name }}</h4>@if (!empty($info->description))<p>{{ Str::limit($info->description, 80) }}</p>@endif">
                    <a href="{{ $imageUrl }}"><img src="{{ $imageUrl }}" alt="{{ $info->name }}" class="img-fluid" loading="lazy"></a>
                </div>
            @empty
                <div class="col-12">
